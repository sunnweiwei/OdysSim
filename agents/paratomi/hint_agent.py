...
"""
ParaToMI hint agent — measures improvement from reflection hints.
...
from agents.paratomi.agent import extract_answer
from agents.paratomi.hint import get_teacher_prompt
...
async def agent_loop(data: dict, context):
    """
    ParaToMI hint agent: re-attempt the question with a hint-augmented prompt.

    Expected extra_info fields (same as paratomi agent, plus):
    ...
    is_correct = correct_answer.lower() in predicted.lower() if correct_answer else False
    new_reward = 1.0 if is_correct else 0.0
    reward_delta = new_reward - old_reward

    extra_info = {
        "paratomi-hint/reward": new_reward,
        "paratomi-hint/reward_delta": reward_delta,
        "paratomi-hint/delta_positive": int(reward_delta > 0),
    }
