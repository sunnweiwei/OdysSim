...
from . import instructions_registry  # noqa: E402
        ...
        for instr_type, instr_class in self.instruction_classes.items():
            for i in range(num_per_type):
                try:
                    ...
                    dataset.append({"instruction_id": [instr_type], "kwargs": [kwargs], "description": description})
                except Exception:
        ...
        for i in range(num_instructions):
            ...
            for _ in range(num_constraints - 1):
                ...
                for candidate in remaining:
                    if all(self.are_instructions_compatible(candidate, selected) for selected in selected_instructions):
            ...
            if instruction_instances:  # Only add if we successfully created instructions
                dataset.append(
                    {"instruction_id": successful_instruction_ids, "kwargs": kwargs_list, "descriptions": descriptions}
                )
